...
This stack mirrors the AWS reference (CloudFront, S3 static site, API Gateway + Lambda, S3 memory, Bedrock) on Azure using **Front Door Standard**, **Blob Storage** (static website + private memory container), **Linux Python 3.11 Functions** (consumption), and **Azure OpenAI** (`gpt-4o`).
...
3. **Check Azure OpenAI quota** in your chosen region (default in `variables.tf` is **West US 2**; adjust `location` / model if needed):
...
- **Standalone app repository:** use `SamuelAdebodun/.github/workflows/deploy.yml` at the repo root (set `APP_ROOT` to `.` and path filters to `infra/**`, `function-app/**`, etc.).
- **This `agents` monorepo:** a root workflow is already added at `.github/workflows/digital-twin-samuel-deploy.yml` so pushes to `main` run the pipeline. A copy of the same workflow is kept under `SamuelAdebodun/.github/workflows/deploy.yml` for when you split the app into its own repo.
